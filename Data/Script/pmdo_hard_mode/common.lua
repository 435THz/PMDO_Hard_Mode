...
HARD_MODE.assignStrongerMoves = function(chara)
    ...
    if SV.hard_mode_processed.Moves[chara.BaseForm.Species] and SV.hard_mode_processed.Moves[chara.BaseForm.Species][chara.BaseForm.Form] and SV.hard_mode_processed.Moves[chara.BaseForm.Species][chara.BaseForm.Form][prevSetString] then
        ...
        local movesets = SV.hard_mode_processed.Moves[chara.BaseForm.Species][chara.BaseForm.Form][prevSetString]
        local moveset = movesets[1]
        if HARD_MODE.isEliteEnemy(chara) then
            moveset = moveset[2]
        end
        for i, move in pairs(moveset) do
            if move ~= "" then
                local enabled = true
        ...
        local egnum  = 1
        local _, moves_elite, moves_level = HARD_MODE.assignBossMoves(chara, tmnum, tutnum, egnum, {sleep_talk = true})
        SV.hard_mode_processed.Moves[chara.BaseForm.Species][chara.BaseForm.Form][prevSetString] = {moves_level, moves_elite}
...
HARD_MODE.isEliteEnemy = function(chara)
    local roll = _DATA.Save.Rand:Next(100)+1
    return chara.Level > roll
end
...
---@return slotType[], string[], string[] #the list of slot types chosen, in the order they were applied, and the moves that have been rolled. Third parameter only contains levelup moves
HARD_MODE.assignBossMoves = function(chara, tm_allowed, tutor_allowed, egg_allowed, blacklist)
    ...
    ---@type table<slotType,string[]> keeps track of what full moves are assigned to what slot type
    local slot_to_moves = {}
    ---@type table<slotType,string[]> keeps track of what level moves are assigned to what slot type
    local slot_to_level_moves = {}
    ---@type string[]
    local move_list = {}
    local level_move_list = {}

    -- select the moves
    for _, slot_type in pairs(shuffled_slots) do
        slot_to_moves[slot_type] = slot_to_moves[slot_type] or {}
        slot_to_level_moves[slot_type] = slot_to_level_moves[slot_type] or {}
        table.insert(slot_to_moves[slot_type], HARD_MODE.moveSelection[slot_type](moveset_table, synergies, allowed))
        if process_level_only then
            table.insert(slot_to_level_moves[slot_type], HARD_MODE.moveSelection[slot_type](level_only, synergies_level_only, allowed_level))
        else
            table.insert(slot_to_level_moves[slot_type], "")
        end
    end
    local slot_to_moves_final = slot_to_moves
    if process_level_only and not HARD_MODE.isEliteEnemy(chara) then
        slot_to_moves_final = slot_to_level_moves
    end
    for _ = 1, 4, 1 do
        chara:DeleteSkill(0)
    end
    ...
    for _, slot_type in pairs(move_slots) do
        local move = slot_to_moves_final[slot_type][1] --FIFO
        local move_b = slot_to_moves[slot_type][1] --FIFO
        local move_l = slot_to_level_moves[slot_type][1] --FIFO
        if move ~= "" then
        ...
        if move_l ~= "" then
            level_move_list[l] = move_l
            l=l+1
        end
        table.remove(slot_to_moves[slot_type], 1) --the slot to moves lists get progressively emptied out
        table.remove(slot_to_level_moves[slot_type], 1) --the slot to moves lists get progressively emptied out
    end
    while k<=4 do
        move_list[k] = ""
        k=k+1
    ...
    if not process_level_only then level_move_list = move_list end
    return shuffled_slots, move_list, level_move_list
end

--- @param tbl {all:moveset_list, level:moveset_supertable,tm:moveset_supertable,tutor:moveset_supertable,egg:moveset_supertable}
--- @return {all:moveset_list, level:moveset_supertable,tm:moveset_supertable,tutor:moveset_supertable,egg:moveset_supertable}
HARD_MODE.stripNonLevel = function(tbl)
    ...
    for category, subtable in pairs(tbl.level) do
        for _, move_data in ipairs(subtable) do
            table.insert(level_table.level[category], move_data)
            if not level_table.all[move_data.ID] then
                level_table.all[move_data.ID] = {cat = {category}, tables = {"level"}, value = move_data}
            else
                table.insert(level_table.all[move_data.ID].cat, category)
            end
        end
    end
    return level_table
    ...
    for _, form in ipairs(stages) do
        for _, phase in ipairs(workPhases) do
            if phase.enabled then
                ...
                for skill_elem in luanet.each(skillList) do
                    if not skill_elem.Level or skill_elem.Level<=chara.Level then
                        ---@type string
                        local skill = skill_elem.Skill
                        ...
                        elseif moveset_table.all[skill] then
                            for _, subtable in ipairs(moveset_table.all[skill].cat) do
                                local add = true
                                for _, elem in ipairs(moveset_table[supertable][subtable]) do
                                    if elem.ID == skill then
                                        add = false
                                        break
                                    end
                                end
